...
# If the RunCoCo::XChar module is not currently defined, fail on any
...
if ! defined?(RunCoCo::XChar) and ! String.method_defined?(:encode)
  RunCoCo.check_for_name_collision(String, "to_xs")
  RunCoCo.check_for_name_collision(Fixnum, "xchr")
end
...
if String.method_defined?(:encode)
  module RunCoCo
    module XChar # :nodoc:
      CP1252_DIFFERENCES, UNICODE_EQUIVALENT = RunCoCo::XChar::CP1252.each.
        inject([[],[]]) {|(domain,range),(key,value)|
          [domain << key,range << value]
        }.map {|seq| seq.pack('U*').force_encoding('utf-8')}
  
      XML_PREDEFINED = Regexp.new('[' +
        RunCoCo::XChar::PREDEFINED.keys.pack('U*').force_encoding('utf-8') +
      ']')
  
      INVALID_XML_CHAR = Regexp.new('[^'+
        RunCoCo::XChar::VALID.map { |item|
          case item
          when Fixnum
  ...
  #
  class Fixnum
    XChar = RunCoCo::XChar if ! defined?(XChar)
    ...
    def xchr(escape=true)
      ...
      case n when *XChar::VALID
        ...
        RunCoCo::XChar::REPLACEMENT_CHAR
      end
    end
  end
